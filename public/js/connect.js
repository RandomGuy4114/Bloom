...
async function submitLocation(location) {
  ...
  const { data, error } = await supabase.rpc("update_connect_location", {
    user_latitude: location.latitude,
    user_longitude: location.longitude,
    user_accuracy_meters: location.accuracy ?? null,
  });
  if (error) {
    console.error("Unable to update Connect location:", error.message);
    return;
  }
...
export async function stopConnect() {
  activeUserId = null;
  lastUploadAt = 0;
  if (BackgroundGeolocation && nativeTrackingStarted) {
    await BackgroundGeolocation.stop();
...
export async function setConnectEnabled(user, enabled) {
  ...
  if (enabled) {
    ...
    const { data, error } = await supabase.rpc("set_connect_enabled", { enabled: true });
    if (error) throw error;
    try {
      await startConnect(user, { requestPermissions: true });
    } catch (trackingError) {
      await supabase.rpc("set_connect_enabled", { enabled: false });
      await stopConnect();
      throw trackingError;
    }
    return data === true;
  ...
  await stopConnect();
  const { data, error } = await supabase.rpc("set_connect_enabled", { enabled: false });
  if (error) throw error;
  return data === false;
}
...
export async function restoreConnect(user) {
  ...
  const { data: profile, error } = await supabase
    ...
    .eq("id", user.id)
    .single();
  if (error || profile?.connect_enabled !== true) return false;
