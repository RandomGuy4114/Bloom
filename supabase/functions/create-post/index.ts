...
Deno.serve(async (request) => {
  ...
  const subcommunityId = String(form.get("subcommunity") ?? "").trim();
  ...
  if (subcommunityId) {
    if (!/^[1-9]\d*$/.test(subcommunityId)) {
      return json({ error: "Invalid sub-community." }, 400);
    }

    const { data: subcommunity, error: subcommunityError } = await userClient
    ...
    if (
      subcommunityError ||
      !subcommunity ||
      subcommunity.community_parent_uid !== communityId ||
      !subcommunity.members?.includes(user.id)
    ) {
      return json({ error: "You cannot post to this sub-community." }, 403);
    }
  }
  ...
  const { data: post, error: insertError } = await adminClient.from("Posts").insert({
    ...
    subcommunity: subcommunityId || null,
    post_type: postType,
